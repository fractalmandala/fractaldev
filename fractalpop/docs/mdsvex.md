...
You can see all of this running in the demo app: [`/mdsvex`](<../apps/demo/src/routes/(docs)/mdsvex/+page.svx>)
(a `.svx` page) and [`/md`](<../apps/demo/src/routes/(docs)/md/+page.md>) (a plain
`.md` page), both highlighted by this hook.
...
the usual `fp__token--<type>` classes. Style it with the `--fp-*` variables — see
the [core theming docs](./core.md#theming). Add a rule for highlighted lines:
...
- Highlight `.md` and `.mdx` on any framework: [`@fractalpop/remark`](./remark.md)
- Use the components directly: [`@fractalpop/svelte`](./svelte.md)
