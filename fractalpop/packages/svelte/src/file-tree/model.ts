/** File-tree model: derive a sorted, nestable item list from flat paths. */
...
/** Build tree items from paths. Directory keys end in a slash; callers' file paths are preserved. */
export function treeItems(paths: readonly string[]): TreeItem[] {
  ...
  function visit(parent: string | null) {
    ...
    siblings.sort(
      (a, b) => Number(b.directory) - Number(a.directory) || (a.name < b.name ? -1 : a.name > b.name ? 1 : 0),
    )
...
/** Filter items hidden under a collapsed ancestor. */
export function visibleItems(items: TreeItem[], collapsed: ReadonlySet<string>): TreeItem[] {
