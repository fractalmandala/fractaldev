...
description: CSS or SASS routes available, understanding the presets, choosing your method.
id: 2
type: Getting Started
---

The system is an npm package.
...
```js
import 'fractalstyler/css';       // expanded
import 'fractalstyler/css/min';   // minified
```
...
runtime at `fractalstyler/presets`:

```js
import { initPresets, setPreset, cyclePreset, getPresetScript } from 'fractalstyler/presets';
...
# or pnpm
pnpm --package=fractalstyler dlx fractalstyler init
```
...
```bash
pnpm add fractalstyler
```

```js
import 'fractalstyler/css';       // compiled - nothing else needed
import 'fractalstyler/css/min';   // minified
```
...
Fractalstyler implements the [agent-plugins.org](https://agent-plugins.org/specification) standard. You can install it directly into your AI coding agent or design tool to enable design token queries, live SASS compilation, and automated component generation:
...
```json
{
  "mcp": {
    "fractalstyler": {
      "type": "local",
      "command": ["npx", "-y", "fractalstyler-mcp"],
      "enabled": true
    }
  }
}
...
```js
import { setTheme, getTheme, themes, initPresets } from 'fractalstyler/presets';
