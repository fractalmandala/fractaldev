---
title: Introduction
description: An intro to the philosophy and intent behind this styling system, how it's built, and who is it for.
id: 1
type: Getting Started
---

Of the greatest, most deliberate design there ever has been - ṛta | ऋत, the order of reality - we have long known:

> yathā piṇḍe tathā brahmāṇḍe | yathā brahmāṇḍe tathā piṇḍe

As above, so below, and vice versa. It is the simplest, most elegant ordering principle of reality. The great design is fractal. And so we ask - can styling systems not be fractal too?
...
But you won't, and that's entirely okay - because there is nothing to adopt. **Fractalstyler ships as plain CSS.** One file, one `<link>`, no build step, no preprocessor, no config. Everything in this documentation - every class, every token, all 76 themes, all four preset axes - works exactly the same either way. Themes and presets are classes and attributes, so they hold up with JavaScript disabled too.
