...
description: Reference guide for Level 3 layout fractals, gridding golden rules, card grids, prose reading measures, aspect frames, and scroll reels.
id: 7
type: Usage Guide
---
...
- **Rule 1 (3 items)**: Scales `$`3 -> 1`$`, never `$`2 + 1`$`.
- **Rule 2 (4 items or multiples of 4)**: Scales `$4`-> 2 > `1$`, never `$3` + `1$`.
- **Rule 3 (6 items)**: Scales `$6` -> 3-> 2 -> `1$` (satisfying both harmonic divisibility rules).
