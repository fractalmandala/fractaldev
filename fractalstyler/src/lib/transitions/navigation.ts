...
import { startViewTransition, createNamedTransition } from '$lib/transitions/view-transitions.js';
...
export function safeResolve(path: string): string {
	// `resolve` is generic over the route union, so `Parameters<typeof resolve>`
	// collapses to `[never]`. Cast to the union itself instead.
