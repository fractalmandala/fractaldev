...
// The DOM lib now ships View Transition types, but not every field we touch
// (`activeViewTransition` is still unshipped in some lib versions). Declare only
// the surface we use, standalone, so it can never conflict with `Document`.
