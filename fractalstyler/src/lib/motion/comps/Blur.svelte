<script lang="ts">
	...
	interface Props {
		...
		opacity?: boolean;
		/** Duration in ms for timing calculations. Default: 500 */
		duration?: number;
		/** Delay before animation starts in ms. Default: 0 */
		delay?: number;
		/** Stagger between targets in ms. Default: 120 */
		stagger?: number;
		/** Easing curve name. Default: 'power1.out' */
		ease?: string;
		/** Scroll start position (e.g. 'top 90%'). Default: 'top 90%' */
		start?: string;
		/** Scroll end position (e.g. 'top 50%'). Default: 'top 50%' */
		end?: string;
		/** Scroll scrub: true to link directly to scroll position, or number for damping. Default: 3 */
	...
	function applyEase(p: number, easeName: string): number {
		switch (easeName) {
			...
			default: return 1 - Math.pow(1 - p, 2);
		}
	}
	...
	$effect(() => {
		...
		const endAnchor = parseAnchor(end, 'top', 50);
		...
		function applyProgress(p: number) {
			...
			targets.forEach((target, i) => {
				...
				if (targetProg >= 0.999) {
					...
					const curBlur = radius * (1 - targetProg);
					const curScale = 1 + (initialScale - 1) * (1 - targetProg);
					target.style.filter = `blur(${curBlur}px)`;
