...
But `.navtree-link` has `padding-block: calc(var(--space-3xs) * ...)` — `--space-3xs` is `0.3125rem` = `5px`. That's `10px` total vertical padding, plus the text line-height. At small font sizes, this may not reach 24px. The system should document that interactive classes meet the 24px minimum, or add a `--control-hit-min` token.
