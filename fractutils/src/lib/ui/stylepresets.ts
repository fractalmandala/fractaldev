export const presetAxes = {
	...
	shell: ['lrf', 'lrfa', 'lr', 'lra', 'lf', 'l', 'f', 'c']
} as const;
...
/**
 * Legacy migration — `shelltype` was a numbered axis (1–7); it is now `shell`,
 * a featural composition (l=nav, r=toc, f=footer). Old persisted values and old
 * API calls are mapped forward. 7 ("Docs") was the role-bound full shell → lrf.
 */
export const SHELL_LEGACY: Record<string, string> = {
	'1': 'lrf', '2': 'lr', '3': 'f', '4': 'c', '5': 'lf', '6': 'l', '7': 'lrfa'
};
...
export function initPresets(): void {
	...
	try {
		...
		// Migrate a legacy numbered shelltype → featural shell before applying.
		if (saved.shelltype && !saved.shell) {
			saved.shell = SHELL_LEGACY[saved.shelltype] ?? presetDefaults.shell;
			delete saved.shelltype;
		}
		for (const axis of Object.keys(presetAxes) as PresetAxis[]) {
...
export function setPreset(axis: PresetAxis, value: string): void {
	if (typeof document === 'undefined') return;
	// Accept a legacy numeric shelltype and map it forward before validating.
	if (axis === 'shell' && value in SHELL_LEGACY) value = SHELL_LEGACY[value];
...
export function getPresetScript(): string {
	...
	return `(function(){try{var v=${axes};var d=${defs};var r=document.documentElement;var s=JSON.parse(localStorage.getItem('${STORAGE_KEY}')||'{}');if(s.shelltype&&!s.shell){var L={'1':'lrf','2':'lr','3':'f','4':'c','5':'lf','6':'l','7':'lrfa'};s.shell=L[s.shelltype];}for(var k in v){var val=s[k];if(val&&val!==d[k])r.setAttribute('data-'+k,val);}var m=localStorage.getItem('${MODE_KEY}');if(m==='dark'||m==='light')r.setAttribute('data-mode',m);}catch(e){}})();`;
}
