# Pāṇinian compositions: the usage reference

> How to write the condensed border–padding–gap classes in markup, exactly what
> each one expands to, and where the grammar stops.
...
- [PANINIAN-COMPOSITIONS.md](./PANINIAN-COMPOSITIONS.md) — philosophy, Sass
  implementation, used-only delivery pipeline, packaging
- `_composition-guide.sass` — the compact Sass-only reference
- `_compositions.sass` — term maps, validation, `selectors()`
- `canonical-markups.md` — shell markup and the `data-shell` composition grammar
